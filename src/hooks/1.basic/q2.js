...
function Q2() {
  ...
  const [p, setp] = useState("제출된 목록이 없습니다");
  const [arr2, setarr2] = useState([]);
  const InputList = useRef("");

  const onAddList = () => {
    setForceRender((prev) => !prev);
    arr.push(InputList.current.value);
  };
  console.log(arr);
  const AddList = () => {
    setarr2(arr.map((e) => e));
  ...
  const changetextcolor = useRef(null);

  const ColorChangeButton = () => {
    changetextcolor.current.style.color = "lightblue";
  };
  ...
  return (
    <>
      ...
      <div>
        ...
        <p>
          <button onClick={AddList}>제출</button>
        </p>
      ...
      <div>
        <h2>문제 2-2</h2>
        <p ref={changetextcolor}>
          이 문구는 아래 버튼을 누르면 색상이 바뀝니다
        </p>
        <button onClick={ColorChangeButton}>변경</button>
      </div>
    </>
  );
}
